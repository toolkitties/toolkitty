<script lang="ts">
  ...
  let { resources = null, currentlySelectedId } = $props();
  ...
  const isAvailableDate = (date: DateValue): boolean => {
    return (
      ...
      availableCalendarDates.some((d: DateValue) => isSameDate(d, date))
    );
  };

  const getTimeSlotsForSelectedDate = (date: CalendarDate) => {
    const selectedDateEntry = availableDatesAndTimes.find(
      (entry: AvailableDateEntry) => {
